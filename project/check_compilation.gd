...
func check_methods(instance: Object, methods: Array, component_name: String) -> void:
	...
	if methods_found == methods.size():
		print("    ✅ Todos los métodos verificados para %s" % component_name)
	else:
		print("    ❌ Métodos faltantes en %s (%d/%d)" % [component_name, methods_found, methods.size()])