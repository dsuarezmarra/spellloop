...
func create_gold_orb(position: Vector2, gold_value: int):
	...
	var orb_scene = preload("res://scenes/pickups/XPOrb.tscn")
	var orb = orb_scene.instantiate()
	# Reusar la clase ExpOrb para facilidad, seteando propiedades de oro
	if orb is Node:
		# If the packed scene uses the ExpOrb class, try to set gold-specific fields
		if orb.has_method("initialize"):
			# We pass gold value in exp_value field and UI will treat it as gold when signalled
			orb.initialize(position, gold_value, player)
		# Mark orb as gold for downstream collection logic
		orb.set_meta("is_gold_orb", true)
		orb.set_meta("gold_value", gold_value)
		orb.z_index = 45
		orb.connect("orb_collected", Callable(self, "_on_exp_orb_collected"))
		get_tree().current_scene.add_child(orb)
		active_orbs.append(orb)
