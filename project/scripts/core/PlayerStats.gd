...
const BASE_STATS: Dictionary = {
	...
	"pickup_range": 100.0,        # Rango de recogida en px (valor absoluto)
...
func _apply_weapon_stat_upgrade(stat_name: String, value: float, operation: String) -> void:
	# Delegar a GlobalWeaponStats
	var gws = _get_global_weapon_stats()
	if gws:
		if operation == "add":
			gws.add_stat(stat_name, value)
		elif operation == "multiply":
			# GlobalWeaponStats suele usar stats planos, pero si soporta mult...
			var current = gws.get_stat(stat_name)
			var diff = (current * value) - current
...
func _apply_weapon_effects_to_global(effects: Array, upgrade_dict: Dictionary) -> void:
	...
	if gws == null:
		...
		print("🔴 [PlayerStats] ERROR: No se encontró GlobalWeaponStats en _apply_weapon_effects_to_global!")
		print("  - global_weapon_stats ref: ", global_weapon_stats)
		print("  - inside tree: ", is_inside_tree())
		if is_inside_tree():
			print("  - group count: ", get_tree().get_nodes_in_group("global_weapon_stats").size())
		return

	# DEBUG: Verificar el upgrade que se va a aplicar
	for eff in effects:
		print("🔵 [PlayerStats] Enviando a GWS: %s %s %.2f" % [eff.get("stat"), eff.get("operation"), eff.get("value", 0)])
