extends Node2D
# Enemy: Araña Venenosa
# Tier 1 (0-5 minutos) - Enemigo con ataque especial de veneno

@export var name = "Araña Venenosa"
@export var slug = "enemy_tier_1_poison_spider"
@export var difficulty_tier = 1
@export var base_hp = 30
@export var base_speed = 65.0
@export var base_damage = 5
@export var xp_drop = 4
@export var size_px = 64
@export var collider_radius = 24.0
@export var attack_type = "special"
@export var attack_rate = 2.2
@export var movement_behavior = "ambush"
@export var sprite_path = "res://assets/sprites/enemies/enemy_tier_1_poison_spider.png"
@export var is_special_variant = false

var current_hp: int
var player_reference: Node
var attack_timer: float = 0.0
var scale_manager: Node
var is_hidden: bool = false
var hide_timer: float = 0.0
var web_cooldown: float = 0.0

func _ready():
	if has_node("Sprite"):
		$Sprite.texture = load(sprite_path)
	scale_manager = get_node("/root/ScaleManager")
	player_reference = get_tree().get_first_node_in_group("player")
	current_hp = calculate_scaled_hp()
	
	if has_node("CollisionShape2D"):
		var shape = CircleShape2D.new()
		shape.radius = collider_radius * scale_manager.current_scale
		$CollisionShape2D.shape = shape

func _physics_process(delta):
	if not player_reference:
		return
	ambush_behavior(delta)
	handle_attack(delta)
	handle_web_attack(delta)

func ambush_behavior(delta):
	var distance_to_player = global_position.distance_to(player_reference.global_position)
	var scaled_speed = base_speed * scale_manager.current_scale
	hide_timer -= delta
	
	# Comportamiento de emboscada
	if distance_to_player > 150.0 * scale_manager.current_scale and not is_hidden:
		if hide_timer <= 0.0:
			is_hidden = true
			modulate.a = 0.3  # Semi-transparente
			hide_timer = randf_range(2.0, 4.0)
	
	if is_hidden:
		# Movimiento sigiloso hacia el jugador
		if distance_to_player <= 80.0 * scale_manager.current_scale:
			is_hidden = false
			modulate.a = 1.0
			# Salto de emboscada
			var direction = (player_reference.global_position - global_position).normalized()
			global_position += direction * scaled_speed * 2.5 * delta
		else:
			var direction = (player_reference.global_position - global_position).normalized()
			global_position += direction * scaled_speed * 0.6 * delta
	else:
		# Movimiento normal
		var direction = (player_reference.global_position - global_position).normalized()
		global_position += direction * scaled_speed * delta

func handle_attack(delta):
	attack_timer -= delta
	if attack_timer <= 0.0:
		var distance_to_player = global_position.distance_to(player_reference.global_position)
		var attack_range = collider_radius * 1.5
		
		if distance_to_player <= attack_range:
			perform_poison_attack()
			attack_timer = attack_rate

func perform_poison_attack():
	if player_reference.has_method("take_damage"):
		var damage = calculate_scaled_damage()
		player_reference.take_damage(damage)
		
		# Aplicar efecto de veneno
		if player_reference.has_method("apply_status_effect"):
			player_reference.apply_status_effect("poison", 3.0, 2)  # 3 segundos, 2 daño/sec

func handle_web_attack(delta):
	web_cooldown -= delta
	if web_cooldown <= 0.0:
		var distance_to_player = global_position.distance_to(player_reference.global_position)
		if distance_to_player <= 120.0 * scale_manager.current_scale and distance_to_player >= 60.0:
			shoot_web()
			web_cooldown = 4.0

func shoot_web():
	# Crear proyectil de telaraña que ralentiza
	var web_scene = preload("res://scenes/enemies/WebProjectile.tscn")
	if web_scene:
		var web = web_scene.instance()
		get_parent().add_child(web)
		web.global_position = global_position
		var direction = (player_reference.global_position - global_position).normalized()
		web.setup(direction, 0, 200.0, "slow")  # Sin daño, efecto de ralentización

func take_damage(amount: int):
	current_hp -= amount
	is_hidden = false  # Salir del modo oculto al recibir daño
	modulate = Color.RED
	modulate.a = 1.0
	var tween = create_tween()
	tween.tween_property(self, "modulate", Color.WHITE, 0.2)
	if current_hp <= 0:
		_on_death()

func _on_death():
	drop_xp(calculate_scaled_xp())
	maybe_drop_item()
	queue_free()

func drop_xp(amount: int):
	var xp_scene = preload("res://scenes/effects/XPOrb.tscn")
	if xp_scene:
		var xp_orb = xp_scene.instance()
		get_parent().add_child(xp_orb)
		xp_orb.global_position = global_position
		xp_orb.xp_value = amount

func maybe_drop_item():
	var drop_chance = 0.08  # Mayor por ser especial
	if is_special_variant:
		drop_chance = 1.0
	if randf() < drop_chance:
		var item_manager = get_node("/root/ItemManager")
		if item_manager and item_manager.has_method("spawn_item_drop"):
			var rarity = ItemManager.ItemRarity.WHITE
			if is_special_variant:
				rarity = ItemManager.ItemRarity.BLUE
			item_manager.spawn_item_drop(global_position, rarity)

func calculate_scaled_hp() -> int:
	var game_manager = get_node("/root/GameManager")
	var minutes_elapsed = 0
	if game_manager and game_manager.has_method("get_elapsed_minutes"):
		minutes_elapsed = game_manager.get_elapsed_minutes()
	var hp = base_hp * (1 + 0.12 * minutes_elapsed) * (1 + 0.25 * (difficulty_tier - 1))
	return int(round(hp))

func calculate_scaled_damage() -> int:
	var game_manager = get_node("/root/GameManager")
	var minutes_elapsed = 0
	if game_manager and game_manager.has_method("get_elapsed_minutes"):
		minutes_elapsed = game_manager.get_elapsed_minutes()
	var damage = base_damage * (1 + 0.09 * minutes_elapsed)
	return int(round(damage))

func calculate_scaled_xp() -> int:
	var xp = xp_drop * (1 + 0.1 * difficulty_tier)
	if is_special_variant:
		xp *= 2
	return int(round(xp))

func _on_spawn(spawn_position: Vector2):
	global_position = spawn_position
	if is_special_variant:
		name = "Araña Reina"
		modulate = Color.PURPLE
		current_hp = int(current_hp * 1.8)
		scale *= 1.4
		web_cooldown = 2.0  # Telarañas más frecuentes