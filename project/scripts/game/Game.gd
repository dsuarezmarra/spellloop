...
# ═══════════════════════════════════════════════════════════════════════════════
# TELEMETRÍA Y AUDITORÍA - Ciclo de vida
# ═══════════════════════════════════════════════════════════════════════════════

func _start_balance_telemetry() -> void:
	"""Iniciar tracking de telemetría para la run actual"""
	_last_telemetry_minute = -1
	...
	# RunAuditTracker (per-weapon/per-enemy tracking)
	if RunAuditTracker and RunAuditTracker.ENABLE_AUDIT:
		RunAuditTracker.start_run(context)
	
	# BalanceTelemetry (global balance logging)
...
func _end_balance_telemetry(end_reason: String = "death") -> void:
	"""Finalizar tracking de telemetría al terminar la run"""
	var context = {
		"end_reason": end_reason,
		"time_survived": game_time,
		"level": run_stats.get("level", 1),
		"phase": wave_manager.get("current_phase") if wave_manager and "current_phase" in wave_manager else 1,
...
func _check_telemetry_minute_snapshot() -> void:
	"""Verificar si ha pasado un minuto y tomar snapshot de telemetría"""
	var current_minute = int(game_time / 60.0)
	if current_minute <= _last_telemetry_minute:
		return
	
	_last_telemetry_minute = current_minute
	
	# Contexto para el snapshot
	var context = {
		"level": run_stats.get("level", 1),
		"kills": run_stats.get("kills", 0),
		"player_stats": {},
	...
	# Recopilar stats del jugador si están disponibles
	if player_stats:
		var ps = {}
		for stat_name in ["max_health", "damage_mult", "move_speed", "armor", "crit_chance"]:
			if stat_name in player_stats:
				ps[stat_name] = player_stats.get(stat_name)
		context["player_stats"] = ps
	
	# Recopilar dificultad si wave_manager existe
	if wave_manager:
		var diff = {}
		if "current_phase" in wave_manager:
			diff["phase"] = wave_manager.current_phase
		if "difficulty_mult" in wave_manager:
			diff["difficulty_mult"] = wave_manager.difficulty_mult
		context["difficulty"] = diff
	
	# RunAuditTracker minute tick
	if RunAuditTracker and RunAuditTracker.ENABLE_AUDIT:
		RunAuditTracker.minute_tick(context)

func add_healing_stat(amount: int) -> void:
	"""Registrar curación en las estadísticas de la partida"""
	run_stats["healing_done"] += amount

func add_damage_stat(amount: int) -> void:
	"""Registrar daño infligido en las estadísticas de la partida"""
