...
func _setup_collision():
	...
	collision_mask = 8  # Only hit enemies (layer 8). Wall collisions removed.
...
func _handle_collision(collider: Node):
	...
	if collider.collision_layer & 8:  # Enemy layer
		_hit_enemy(collider)
	# Walls removed: other collision types are ignored here
...
## Wall handling removed — function deleted to avoid stale stubs.
