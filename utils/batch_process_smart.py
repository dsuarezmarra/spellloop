#!/usr/bin/env python3
"""
batch_process_smart.py
Procesa las decoraciones con detección inteligente de frames.
"""

from pathlib import Path
from fix_spritesheet_smart import process_spritesheet_smart
...
def main():
    ...
    print("\n" + "="*70)
    print("PROCESAMIENTO INTELIGENTE - DECORACIONES LAVA")
    print("="*70)
    ...
    for source_name, target_name, expected_frames, description in file_mapping:
        ...
        print(f"\n{'─'*70}")
        print(f"📝 {description}")
        print(f"   Origen: {source_name}")
        print(f"   Destino: {target_name}")
        print(f"   Frames: {expected_frames}")
        print(f"{'─'*70}")
        
        try:
            result = process_spritesheet_smart(
                str(source_path),
                str(final_path),
                expected_frames,
                frame_size=256,
