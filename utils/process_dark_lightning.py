"""
Script para procesar sprites de dark_lightning
...
INPUT_DIR = r"C:\git\spellloop\project\assets\sprites\projectiles\fusion\dark_lightning"
...
def process_animation(name, config):
    ...
    output_path = os.path.join(OUTPUT_DIR, f"{name}_spritesheet_dark_lightning.png")
...
def main():
    print("="*60)
    print("PROCESADOR DE SPRITES - DARK LIGHTNING")
    print("="*60)
    ...
    for name in ANIMATIONS:
        path = os.path.join(OUTPUT_DIR, f"{name}_spritesheet_dark_lightning.png")
        if os.path.exists(path):
            img = Image.open(path)
            p = np.array(img)
            opaque = np.sum(p[:, :, 3] > 30)
            print(f"  {name}_spritesheet_dark_lightning.png: {img.size}, {opaque} px opacos")
