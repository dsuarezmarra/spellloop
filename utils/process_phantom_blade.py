"""
Script para procesar sprites de phantom_blade
...
INPUT_DIR = r"C:\git\spellloop\project\assets\sprites\projectiles\fusion\phantom_blade"
...
def process_animation(name, config):
    ...
    output_path = os.path.join(OUTPUT_DIR, f"{name}_spritesheet_phantom_blade.png")
...
def main():
    print("="*60)
    print("PROCESADOR DE SPRITES - PHANTOM BLADE")
    print("="*60)
    ...
    for name in ANIMATIONS:
        path = os.path.join(OUTPUT_DIR, f"{name}_spritesheet_phantom_blade.png")
        if os.path.exists(path):
            img = Image.open(path)
            p = np.array(img)
            opaque = np.sum(p[:, :, 3] > 30)
            print(f"  {name}_spritesheet_phantom_blade.png: {img.size}, {opaque} px opacos")
