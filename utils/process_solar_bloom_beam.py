#!/usr/bin/env python3
"""
Procesa los sprites de Solar Bloom (BEAM):
- beam_body_solar_bloom.png - tileable horizontal desde imagen fuente
"""
...
def process_beam_body(source_path, output_path, name):
    ...
    # Si la imagen es un grid 2x3, extraer frames
    # De lo contrario, usar la imagen directamente
    if w > h * 1.2:  # Es probablemente un grid horizontal
        frames = extract_2x3_frames(source_path)
        valid_frames = [f for f in frames if f is not None]
        
        if valid_frames:
            print(f'Frames extraídos del grid: {len(valid_frames)}')
            ...
            content_source = best_frame
        else:
            content_source = img
    else:
        # Usar la imagen completa
        content_source = img
        print('Usando imagen completa (no es grid)')
    
    # Calcular el bounding box real del contenido
    arr = np.array(content_source)
    alpha = arr[:, :, 3]
    ...
    if np.any(rows_with) and np.any(cols_with):
        ...
        content = content_source.crop((x1, y1, x2 + 1, y2 + 1))
    else:
    ...
    # Asegurar mínimo de ancho
    if new_width < 32:
        new_width = 32
        scale_factor = 32 / content.width
        target_height = int(content.height * scale_factor)
...
if __name__ == "__main__":
    base = Path(r'C:\git\spellloop\project\assets\sprites\projectiles\fusion\solar_bloom')
    ...
    if body_source.exists():
        body_result = process_beam_body(body_source, base, 'solar_bloom')
    else:
        print(f'❌ No existe: {body_source}')
    
    # Procesar beam_tip si existe un archivo tip
    # Buscar archivo tip
    tip_candidates = ['tip.png', 'beam_tip_solar_bloom.png']
    tip_source = None
    for candidate in tip_candidates:
        path = base / candidate
        if path.exists():
            # Verificar si es un grid 2x3 (el tip debe tener múltiples frames)
            img = Image.open(path)
            if img.width > img.height * 1.2:  # Parece ser un grid
                tip_source = path
                break
    
    if tip_source:
        tip_result = process_beam_tip(tip_source, base, 'solar_bloom')
    else:
        print(f'\n⚠️ No se encontró archivo tip válido para procesar')

    print("\n" + "="*60)
    print("RESUMEN - SOLAR BLOOM")
    print("="*60)
    if body_result:
        print(f"beam_body_solar_bloom.png: {body_result['width']}x{body_result['height']}")
    if tip_result:
        print(f"beam_tip_solar_bloom.png: {tip_result['sheet_width']}x{tip_result['frame_size']} ({tip_result['num_frames']} frames)")
        print(f"\nConfig para ProjectileVisualManager.gd:")
        print(f'"solar_bloom": {{"beam_frame_size": {tip_result["frame_size"]}, ...}}')
