"""
Script para procesar sprites de soul_reaper
...
INPUT_DIR = r"C:\git\spellloop\project\assets\sprites\projectiles\fusion\soul_reaper"
...
def process_animation(name, config):
    ...
    output_path = os.path.join(OUTPUT_DIR, f"{name}_spritesheet_soul_reaper.png")
...
def main():
    print("="*60)
    print("PROCESADOR DE SPRITES - SOUL REAPER")
    print("="*60)
    ...
    for name in ANIMATIONS:
        path = os.path.join(OUTPUT_DIR, f"{name}_spritesheet_soul_reaper.png")
        if os.path.exists(path):
            img = Image.open(path)
            p = np.array(img)
            opaque = np.sum(p[:, :, 3] > 30)
            print(f"  {name}_spritesheet_soul_reaper.png: {img.size}, {opaque} px opacos")
